...
double ContractionSeqOptimizerPEPS::determineContractionSequence(TensorNetwork & network,
                                                                  ...
                                                                  std::function<unsigned int ()> intermediate_num_generator)
{
 ...
 std::cout << "EXATN-PEPS: printing the peps " << std::endl;
 network.printIt();
 if(num_tensors > 1){
...
std::unique_ptr<ContractionSeqOptimizer> ContractionSeqOptimizerPEPS::createNew()
{
 return std::unique_ptr<ContractionSeqOptimizer>(new ContractionSeqOptimizerPEPS());
}
